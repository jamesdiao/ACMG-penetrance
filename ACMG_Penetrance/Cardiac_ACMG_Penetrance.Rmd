...
By Bayes' Rule, the penetrance of a variant related to disease $x$ may be defined as: $$P(D_x|V_x) = \frac{P(D_x)*P(V_x|D_x)}{P(V_x)} = \frac{(Prevalence)(Case \; Allele \; Frequency)}{(Population \; Allele \; Frequency)}$$
...
sample_beta <- function(freq, n, trials) {
  ...
  max_prev_n <- prev_n #quantile(prev_n, 0.5)
  max_caf_n <- caf_n #quantile(caf_n, 0.5)
  sample.prev <- sample_beta(freq = prev_obs/prev_n %>% setNames(abbrev), n = pmin(prev_n, max_prev_n))
  sample.CAF <- sample_beta(freq = caf_obs/caf_n %>% setNames(abbrev), n = pmin(caf_n, max_caf_n))
...
plot(sample.prev %>% 
  ...
  geom_density(alpha = 0.1, adjust = 2) + 
  coord_cartesian(xlim=c(0, min(1,1.5*max(apply(sample.prev, 2, mean))))) +
...
plot(sample.CAF %>% 
  ...
  geom_density(alpha = 0.1, adjust = 2) + xlim(0,1) + 
