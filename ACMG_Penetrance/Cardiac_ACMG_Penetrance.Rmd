...
#save.image("/Users/jamesdiao/Documents/Kohane_Lab/2017-ACMG-penetrance/Cardiac_Environ_2017-06-23.RData")
load("/Users/jamesdiao/Documents/Kohane_Lab/2017-ACMG-penetrance/Cardiac_Environ_2017-06-23.RData")
```
...
colnames(af_1000g_by_ancestry) <- c("AFR","AMR","EAS","EUR","SAS")
...
ggplot(ranked_var, aes(x = Var_ID, y = Subdivided_Allele_Frequencies, fill = Ancestry)) +
    ...
    ggtitle("Max-Population Variants in 1000 Genomes, No Conflicts") + coord_flip()

### gnomAD
af_gnomad_by_ancestry <- merged_gnomad[,sprintf("AF_GNOMAD_%s",
                            c("AFR","AMR","ASJ","EAS","FIN","NFE","SAS","OTH"))]
colnames(af_gnomad_by_ancestry) <- c("AFR","AMR","ASJ","EAS","FIN","NFE","SAS","OTH")
...
sample.freqs <- lapply(1:length(sample_size$Cohort_GNOMAD), function(x) {
  ...
  if (subset=='GNOMAD') {
    ...
    col <- sprintf("AF_GNOMAD_%s",subset)
  }
...
y <- data.frame(Ancestry = rep(names(sample.freqs), each = nrow(sample.freqs[[1]])),
                do.call('rbind', sample.freqs))
plot(y %>% gather(key = Disease, value = Frequency, -Ancestry) %>%
  filter(Ancestry != 'GNOMAD') %>% 
  ggplot(aes(Frequency, color = Ancestry, fill = Ancestry)) + geom_density(alpha = 0.1) +
  theme(axis.text.x = element_text(angle = 30, hjust = 1)) + 
