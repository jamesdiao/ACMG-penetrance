...
#save.image("/Users/jamesdiao/Documents/Kohane_Lab/2016-paper-ACMG-penetrance/Environ_2016-12-07.RData")
load("/Users/jamesdiao/Documents/Kohane_Lab/2016-paper-ACMG-penetrance/Environ_2016-12-07.RData")
```
...
var_plot_1000g <- function(pathogenic, frac) {
  ACMG.data <- parse(text=paste0(ifelse(pathogenic, "merged_", "ACMG."), "1000g")) %>% eval
  ...
  sapply(pop.levels, function(pop) {
    if (frac) {
      ...
      temp <- colSums(ACMG.data[,c(front_cols,map$pop)==pop]>0)
    }
    c(mean(temp), sd(temp))
  ...
  values$Population <- factor(pop.levels, levels = pop.levels)
  values$Superpopulation <- factor(super[pop.levels], levels = super.levels)
  title <- sprintf("ACMG-56%s: Mean in 1000 Genomes", ifelse(pathogenic, " Pathogenic",""))
...
var_plot_exac <- function(dataset, pathogenic, frac) {
  ...
  ggplot(exac_values, aes(x = Superpopulation, y=values, fill = Superpopulation)) + 
    ...
    ggtitle(sprintf("ACMG-56%s: Mean in %s", ifelse(pathogenic, " Pathogenic",""), dataset)) + 
    ...
    sapply(c(dataset,super.levels), function(superpop) {
      if (ind) {
        ...
        if (inheritance[item] == "XL")
          freq <- freq/2 #XL: prob of having a non-reference site at ONE allele (male)
...
freq_gnomad.calc.gene <- getAlleleFreq(merged_gnomad, ind = T, method = "Gene", dataset = "GNOMAD")
...
ratio_diff <- function() {
  ratio <- data.frame(Means = allele.freq$COUNT_1000G, 
    ...
    mutate(Disease = factor(abbrev, 
           levels = abbrev[order(pmax(Ratio_gnomAD,Ratio_1000G))])) %>% 
...
ancestry_penetrance <- function(ah_low, ah_high, dataset, range, position) {
  ...
  sapply(c("Total",super.levels), function(superpop){
    ...
    named.freqs <- parse(text=sprintf("freq_%s.calc.gene", tolower(dataset))) %>% eval
    if (superpop != "Total") 
      find <- paste(find, superpop, sep = "_")
