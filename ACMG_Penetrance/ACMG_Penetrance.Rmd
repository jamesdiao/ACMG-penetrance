...
#save.image("/Users/jamesdiao/Documents/Kohane_Lab/2016-paper-ACMG-penetrance/Environ_2016-12-04.RData")
load("/Users/jamesdiao/Documents/Kohane_Lab/2016-paper-ACMG-penetrance/Environ_2016-12-04.RData")
```
...
for (gene in ACMG.panel) {
  #print(sprintf("[%d/%d] %s",which(gene==ACMG.panel),length(ACMG.panel),gene))
  ACMG.exac <- rbind(ACMG.exac,import.file.exac(gene, use_exac))
}
#Display and remove duplicates
...
#cat(sprintf("After filtering for coupling/bad-locations/conflicting-claims: %s variants (FINAL)", nrow(clinvar_query)))
...
clinvar_count[4] <- sum(clinvar_query$VAR_ID %in% ACMG.exac$VAR_ID)
#cat(sprintf("Overlap with ClinVar VCF: %s variants", sum(clinvar_query$VAR_ID %in% clinvar$VAR_ID)))
clinvar_count[5] <- sum(clinvar_query$VAR_ID %in% clinvar$VAR_ID)
#cat(sprintf("Overlap with LP/P ClinVar VCF: %s variants", sum(clinvar_query$VAR_ID %in% clinvar$VAR_ID[clinvar$INTERP])))
clinvar_count[6] <- sum(clinvar_query$VAR_ID %in% clinvar$VAR_ID[clinvar$INTERP])
#cat(sprintf("Overlap with LP/P ClinVar VCF AND ExAC: %s variants", sum(clinvar_query$VAR_ID %in% merged_exac$VAR_ID)))
clinvar_count[7] <- sum(clinvar_query$VAR_ID %in% merged_exac$VAR_ID)
...
clinvar_count[8] <- sum(clinvar_query$VAR_ID %in% merged_1000g$VAR_ID)
#merged_1000g[merged_1000g$VAR_ID %in% clinvar_query$VAR_ID, c(3,1:2,6,10)][1:3,]
clinvar_count[9] <- sum((clinvar_query$VAR_ID %in% merged_exac$VAR_ID) & (clinvar_query$VAR_ID %in% merged_1000g$VAR_ID))
...
data.frame(Subset = c("Initial Count","Filter Substitutions (N>N')","Filter Coupling/Bad-Locations","In ExAC","In ClinVar VCF","In LP/P-ClinVar VCF","^ & ACMG & ExAC","^ & ACMG & 1000 Genomes", "^ & ACMG & ExAC & 1000 Genomes"), Number_of_Variants = clinvar_count) %>% pander
...
ggplot(aes(x = c(1:20,1:20), y = Frequency)) + 
...
ACMG_Lit <- read.csv(file = "Supplementary_Files/Literature_Prevalence_Estimates.csv", 
              header = TRUE, stringsAsFactors = F, na.strings = "\\N") %>% filter(Evaluate)
    ...
    sapply(c(dataset,super.levels), function(superpop) {
      if (ind) {
        ...
        if (inheritance[item] == "XL")
          freq <- freq*2/3 #XL: prob of having a non-reference site at ONE allele (male)
        # 2*AC_m = AC_f, AC_m/AN_m = 2/3*(recorded freq)
...
pen_exac <- get_penetrance(ah_low = 0.01, ah_high = 1, dataset = "ExAC")
#ah_low = 0.01; ah_high = 1; dataset = "ExAC"
```
<br />
...
make_prev_ranges <- function(range) {
    sapply(ACMG_Lit$Inverse_Prevalence, function(prev){
    if (length(prev)>1) {
...
get_penetrance <- function(ah_low, ah_high, dataset, range) {
  ...
  # Take row 5 to sort by max, take colSums to sort by overall
  ord <- order(penetrance[5,], decreasing = T)
  # replicate each element n times to create labels
  ...
  plot(ggplot(aes(x=Disease, y=Penetrance), data = penetrance_data) + 
    geom_boxplot(position = 'identity', coef = 0, na.rm = T) + coord_flip() + 
      ggtitle(sprintf("%s: Barplot of Min/Mean/Max Penetrance", dataset)) + 
      theme(axis.text.y = element_text(color = colormap))
  ) 
  penetrance_data
}
