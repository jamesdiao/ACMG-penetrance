...
#save.image("/Users/jamesdiao/Documents/Kohane_Lab/2016-paper-ACMG-penetrance/Environ_2016-12-11.RData")
load("/Users/jamesdiao/Documents/Kohane_Lab/2016-paper-ACMG-penetrance/Environ_2016-12-11.RData")
```
...
get_clinvar <- function(clinvar_file) {
  ...
  input <- input %>% filter(filter_condition) %>%
    ...
    mutate(CLNREVSTAT = strsplit(CLNREVSTAT,"|",fixed = T)) %>% 
...
var_plot_1000g <- function(pathogenic, frac) {
  ...
  values$Population <- factor(pop.levels, levels = pop.levels)
  values$Superpopulation <- factor(super[pop.levels], levels = super.levels)
  title <- sprintf("ACMG-56%s: %s in 1000 Genomes", 
...
var_plot_exac <- function(dataset, pathogenic, frac) {
  ...
  ggplot(exac_values, aes(x = Superpopulation, y=values, fill = Superpopulation)) + 
    ...
    ggtitle(sprintf("ACMG-56%s: %s in %s", 
              ifelse(pathogenic, " Pathogenic",""), dataset), ifelse(frac,"Fraction","Mean")) + 
...
aggregateCalc <- function(input, superpop, item, dataset, loc) {
  ...
  1-prod(1-freq[!is.na(freq)]) # prob of having a non-reference site in 1 chrom
}
...
aggregateCount <- function(input, superpop, item, dataset, loc) {
  ...
  mean(apply(reduced_input, 2, function(col) any(col>=1)))
}
