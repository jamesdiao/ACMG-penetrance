...
#save.image("/Users/jamesdiao/Documents/Kohane_Lab/2017-ACMG-penetrance/LMM_Environ_2017-06-20.RData")
#load("/Users/jamesdiao/Documents/Kohane_Lab/2017-ACMG-penetrance/LMM_Environ_2017-06-20.RData")
```
...
scrape_MedGen <- function() {
  ACMG.page <- getURL("https://www.ncbi.nlm.nih.gov/clinvar/docs/acmg/", 
                      ssl.verifyhost=FALSE, ssl.verifypeer=FALSE)
  links <- strsplit(ACMG.page,'href=\\"')[[1]]
  (links[grep('medgen/C', links)]) %>% strsplit("/medgen/") %>% sapply("[[",2) %>% unlist %>%
    strsplit('\\">MedGen') %>% sapply("[[",1) %>% unlist
}
medgen <- scrape_MedGen()

HCM.panel <- c("ACTC1", "ACTN2", "CSRP3", "GLA", "LAMP2", "MYBPC3", "MYH7", 
"MYL2", "MYL3", "MYOZ2", "NEXN", "PLN", "PRKAG2", "PTPN11", "RAF1", 
"TNNC1", "TNNI3", "TNNT2", "TPM1", "TTR")

random.panel <- c("TAF6", "IGFN1", "LGALS17A", "ATXN7", "COX6A1", "AA06", "PXYLP1", 
                  "PPP1R8", "SNORD2", "EID3", "CCDC190", "TARM1", "ABCA11P", "PRICKLE3", 
                  "NDP", "RIPK2", "SLC25A24", "ZNF580", "MTR", "RLN3", "FLJ30679", 
                  "RPL30", "CDHR5", "SNX11", "EPHB3", "DAOA", "CFTR", "CDK15", 
                  "LRRC37A3", "DGKZ", "DAP3", "WDR11", "POU4F3", "DTX2", "PIP4K2B", 
                  "KCNMB4", "CCDC134", "MROH8", "ZDHHC11", "PCDHGC5", "KCNIP3", 
                  "PRPSAP2", "EMID1", "SRFBP1", "NR4A1", "THSD7B", "ZNF681", "SNORD55", 
                  "IREB2", "PPP1R3A", "RUVBL1", "CLEC2D", "HOXB9", "LZIC", "TYRO3", 
                  "PNKD", "PLXNB3", "TOMM5", "NINL", "KCMF1", "LRFN2", "QTRT1", 
                  "SLC18A2", "DUXAP8", "RPL23AP53", "OR8J1", "VTN", "FANCG", "INF2", 
                  "NMNAT1", "RAPGEF1", "GH2", "HHIPL1", "ZC3H12C", "CCDC89", "CPSF4L", 
                  "RUNX3", "NUDT9", "TPTE2P1", "KIF16B", "CLUAP1", "IL17B", "TFF3", 
                  "SNAP23", "OCIAD1", "ZNF271P", "PPFIA2", "AIF1L", "ORAI1", "UCHL5", 
                  "METTL7B", "ZNF362", "ENTPD6", "CCL19", "NYNRIN", "NANOS3", "CATSPER2P1", 
                  "SPAG11B", "OTUD6A", "KLHL5")
```
...
get_mc_clinvar <- function(clinvar_file) {
  ...
  input <- input %>% unite(VAR_ID, CHROM, POS, REF, ALT, sep = "_", remove = F) %>% 
    select(VAR_ID, CHROM, POS, ID, REF, ALT, CLNSIG, INTERP, GOLD_STARS, LMM, everything())
}
...
clinvar[5:8,] %>% select(-CLNALLE, -INFO, -QUAL, -FILTER, -CLNDSDB) %>% remove_rownames %>% pander
unlink(clinvar_file)
```
<br />
...
download_1000g <- function(gene, download) {
  ...
  if (nrow(UCSC) == 0) { #No hit on refGene
    ...
    file.stat <- unlist(sprintf("stat %s_genotypes.vcf", gene) %>% system(intern = T) %>% strsplit(" "))
...
if (!skip_download & !skip_processing) {
  ...
  download_output <- sapply(random.panel, function(gene) download_1000g(gene, download = T)) %>% t
...
```{r import_1000g}
import_file_1000g <- function(gene, panel) {
  #for tracking: 
  sprintf("%s [%s/%s]", gene, grep(gene, panel), length(panel)) %>% print(quote = F)
  name <- paste("Random_1000G",paste(gene,"genotypes.vcf", sep = "_"), sep = "/")
...
if (skip_processing) {
  ...
  for (gene in random.panel) {
    #print(sprintf("[%d/%d] %s",which(gene==ACMG.panel),length(ACMG.panel),gene))
    ACMG.1000g <- rbind(ACMG.1000g,import_file_1000g(gene, random.panel))
  }
...
#save(ACMG.1000g, file = "Random.1000G")
...
import_file_exac <- function(gene, dataset) {
  file_name <- sprintf("HCM_%s/%s_%s.csv", dataset, dataset, gene)
...
collect <- clinvar$pathogenic #clinvar$NOT_OMIM_INTERP #& clinvar$LMM_INTERP #& (clinvar$GOLD_STARS >= 2) 
#INTERP means at least 1 (likely) pathogenic and no (likely) benign


merge_clinvar_1000g <- function() {
  inter <- intersect(clinvar$VAR_ID[collect], ACMG.1000g$VAR_ID)
  ...
  cbind(ACMG_merged[,c("GENE",sprintf("AF_1000G%s", c("",paste0("_",super.levels))))], 
        clinvar_merged,ACMG_merged[,-front_cols]) %>% return()
}
merged_1000g <- merge_clinvar_1000g()

inter <- intersect(clinvar$VAR_ID[collect], ACMG.exac$VAR_ID)
...
inter <- intersect(clinvar$VAR_ID[collect], ACMG.gnomad$VAR_ID)
...
<br />
```{r af_distribution, fig.height = 4}
...
Despite some upward concavity, the plot demonstrates reasonable Poissonness, with correlation = `r signif(cor(k,log(x)+lfactorial(k)),2)`.  
```{r poissonness, fig.height = 3, fig.width = 3}
ggplot(aes(x=k,y=poissonness), data = poissondata) + geom_point() + 
  ylab("log(x_k) + log(k!)") + ggtitle("Poissonness Plot")
```
<br /> 
...
ACMG_Lit[c(1,7,11,18),] %>% remove_rownames %>% 
  select(Gene, Phenotype, Inverse_Prevalence, Case_Allele_Frequency) %>% pander
...
freq_1000g.count.mim <- getAlleleFreq(input = merged_1000g, ind = F, dataset = "1000G", method = "MIM")
freq_1000g.calc.mim <- getAlleleFreq(input = merged_1000g, ind = T, dataset = "1000G", method = "MIM")
freq_gnomad.calc.mim <- getAlleleFreq(input = merged_gnomad, ind = T, dataset = "GNOMAD", method = "MIM")
freq_exac.calc.mim <- getAlleleFreq(input = merged_exac, ind = T, dataset = "EXAC", method = "MIM")

freq_1000g.count.medgen <- getAlleleFreq(input = merged_1000g, ind = F, dataset = "1000G", method = "MedGen")
freq_1000g.calc.medgen <- getAlleleFreq(input = merged_1000g, ind = T, dataset = "1000G", method = "MedGen")
freq_gnomad.calc.medgen <- getAlleleFreq(input = merged_gnomad, ind = T, dataset = "GNOMAD", method = "MedGen")
freq_exac.calc.medgen <- getAlleleFreq(input = merged_exac, ind = T, dataset = "EXAC", method = "MedGen")
...
save(freq_1000g.calc.gene, freq_1000g.count.gene, freq_exac.calc.gene, freq_gnomad.calc.gene, 
     ...
     file = "Supplementary_Files/Processed_Files/disease_level_AF.RData")
...
```{r MIM_vs_gene, fig.height = 3.5, fig.width = 4}
plot_af_relation <- function(input1, input2, name1, name2) {
  data.frame(GENE = log10(input1), MIM = log10(input2), LABEL = ACMG_Lit_Full$Abbreviation) %>% 
  ggplot(aes(x = GENE, y = MIM, label = LABEL)) + 
    geom_point() + geom_text(aes(label=LABEL),hjust=0, vjust=0, size = 3) + 
    geom_abline(slope = 1, intercept = 0) + ggtitle(sprintf("%s vs. %s", name2, name1)) + 
    xlab(sprintf("AF using %s",name1)) + ylab(sprintf("AF using %s", name2))
}
plot_af_relation(input1 = freq_gnomad.calc.gene$AF_GNOMAD, input2 = freq_gnomad.calc.mim$AF_GNOMAD,
                 name1 = "GENE", name2 = "MIM")
```

```{r MedGen_vs_gene, fig.height = 3.5, fig.width = 4}
plot_af_relation(input1 = freq_gnomad.calc.gene$AF_GNOMAD, input2 = freq_gnomad.calc.medgen$AF_GNOMAD,
                 name1 = "GENE", name2 = "MedGen")

```

```{r identify_tags}
get_orpha_genes <- function() {
  unlist(merged_gnomad$CLNDSDBID) -> all_id
  all_id <- all_id[grepl("ORPHA",all_id)]
  all_id <- all_id %>% strsplit("ORPHA") %>% lapply("[[",2) %>% unlist %>%
    strsplit(":", fixed = T) %>% lapply("[[",1) %>% unlist %>% 
    as.integer() %>% table %>% sort
  all_names <- sprintf("ORPHA%s", names(all_id))
  lapply(all_names, function(name) {
    merged_gnomad$GENE[grep(name, merged_gnomad$CLNDSDBID)] %>% 
      as.character %>% setNames(NULL) %>% table
  }) %>% setNames(all_names)
}
#orpha_genes <- get_orpha()

get_medgen_genes <- function() {
  all_medgen <- (merged_gnomad$CLNDSDB %>% unlist %>% strsplit(":") %>% unlist) == "MedGen"
  all_id <- merged_gnomad$CLNDSDBID %>% unlist %>% strsplit(":") %>% unlist
  all_id <- all_id[all_id!="HP"][medgen] %>% table %>% sort
  all_names <- names(all_id)
  output <- lapply(all_names, function(name) {
    merged_gnomad$GENE[grep(name, merged_gnomad$CLNDSDBID)] %>% 
      as.character %>% setNames(NULL) %>% table
  }) %>% setNames(all_names)
}
#medgen_genes <- get_all_medgen()

medgen_numbers <- lapply(medgen, function(name) {
  grepl(name, merged_gnomad$CLNDSDBID) %>% sum
}) %>% setNames(medgen) %>% unlist
```
...
ratio_diff <- function() {
  ...
  ggplot(aes(x=Disease, y=Ratio, color = Dataset), data = ratio) + coord_flip() + 
    ...
    scale_y_continuous(breaks = seq(0,10000,20)) + theme(legend.position="none")
}
ratio_diff()
```
...
plot_ind_test <- function() {
  ...
  #               ind = F, dataset = "1000G", method = "gene")$AF_1000G, 
  #             CALC = getAlleleFreq(ACMG.1000g[select_rows,], 
  #               ind = T, dataset = "1000G", method = "gene")$AF_1000G)
  #})) -> ind_test_data
  ...
  plot(plot_data %>% filter(!(Disease %in% c("Wilson","HCRC.AR","OTC","Fabry"))) %>% 
  ...
  plot_data <- ind_test_data[runif(nrow(ind_test_data)) < 0.01,]
  plot_data$AR_XL <- plot_data$DISEASE %in% c("Wilson","HCRC.AR","OTC","Fabry")
  
  plot(ggplot(aes(x = COUNT, y = CALC), data = plot_data) +
         geom_point(col = ifelse(plot_data$AR_XL,'red','black')) + 
         ggtitle("Testing Independence with Random Sampling") + 
         scale_x_continuous(limits = c(0,0.3), breaks = seq(0,1,0.1)) + 
  ...
  cat("31 diseases x 1000 points = 31,000 points.")
...
ancestry_pen <- ancestry_penetrance(ah_low = 0.01, ah_high = 1, dataset = "gnomAD", range = 5, position = "Max", af_type = "gene")
...
```{r ancestry_comp, fig.width = 10, eval = F} 
sapply(super.levels, function(superpop){
  freq_gnomad.calc.gene %>% select(contains(superpop)) %>% unlist %>% setNames(NULL)
}) -> AF_frame
mult_range <- AF_frame %>% apply(1, function(row) max(row)/min(row)) %>% 
  setNames(ACMG_Lit_Full$Short_Name) %>% sort
#diff_range <- AF_frame %>% apply(1, function(row) diff(range(row))) %>% 
#  setNames(ACMG_Lit_Full$Short_Name) %>% sort
diff_range <- ancestry_pen[-c(1,ncol(ancestry_pen))] %>% 
  apply(1, function(row) diff(range(row))) %>% 
  setNames(ACMG_Lit$Short_Name) %>% sort
```
...
```{r, eval = F}
lapply(1:nrow(download_output), function(i) {
  sum((clinvar$CHROM == download_output$chrom[i]) & between(clinvar$POS, download_output$start[i], download_output$end[i]) & clinvar$INTERP)
}) %>% setNames(ACMG.panel) %>% unlist -> cv_num
```

```{r, eval = F, include = F}
LMM.panel <- c("ACTC1", "ACTN2", "CSRP3", "GLA", "LAMP2", "MYBPC3", "MYH7", "MYL2", "MYL3", "MYOZ2", "NEXN", "PLN", "PRKAG2", "PTPN11", "RAF1", "TNNC1", "TNNI3", "TNNT2", "TPM1", "TTR")

for (con in dbListConnections(MySQL())) dbDisconnect(con)
con <- dbConnect(MySQL(), user = 'genome',
                 dbname = 'hg19', host = 'genome-mysql.cse.ucsc.edu',
                 unix.sock = "/Applications/MAMP/tmp/mysql/mysql.sock")
suppressWarnings(dbGetQuery(con, "select name2 from refGene")) -> random.genes
sample(random.genes,1000) -> x
no_no <- grepl("orf",x) | grepl("LOC",x) | grepl("LINC",x) | grepl("MIR",x) | 
  grepl("HIST",x) | grepl("-",x)
x[!no_no][1:110] -> random
```
...
```{r, eval = F, include = F}
clinvar$pathogenic_no_conflicts <- clinvar$INTERP
clinvar$pathogenic_incl_conflicts <- sapply(clinvar$CLNSIG, function(x) any(x %in% c(4,5)))

input <- input %>%
  rename(mc_pathogenic_incl_conflicts = pathogenic_incl_conflicts, 
         mc_pathogenic_no_conflicts = pathogenic_no_conflicts) %>% 
  unite(VAR_ID, chr, pos, ref, alt, remove = F)

any_path <- intersect(clinvar$VAR_ID[clinvar$pathogenic_incl_conflicts], input$VAR_ID)
unconf_path <- intersect(clinvar$VAR_ID[clinvar$pathogenic_no_conflicts], input$VAR_ID)
output <- input %>% 
            mutate(pathogenic_no_conflicts = VAR_ID %in% unconf_path) %>%
            mutate(pathogenic_incl_conflicts = VAR_ID %in% any_path) %>%
            select(chr = chr, pos = pos, ref = ref, alt = alt, rsID = rsID, 
                   contains("pathogenic_"), #contains("HG"), contains("NA"))
                     contains("Count"), contains("Number"))
write.csv(output, file = "HCM_gnomAD_Variants_7_11.csv", row.names = F, quote = F)
