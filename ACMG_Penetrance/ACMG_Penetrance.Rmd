...
#save.image("/Users/jamesdiao/Documents/Kohane_Lab/2017-ACMG-penetrance/Environ_2017-01-25.RData")
load("/Users/jamesdiao/Documents/Kohane_Lab/2017-ACMG-penetrance/Environ_2017-01-25.RData")
```
...
report <- ACMG_Lit_Full$Variants_to_report %>% expand_pipes()
  ...
  report.mg <- setNames(report, MG.list)[!duplicated(MG.list)]
inheritance <- ACMG_Lit_Full$Inheritance %>% expand_pipes()
  ...
  inheritance.mg <- setNames(inheritance, MG.list)[!duplicated(MG.list)]
...
(1) By associating it with an MIM. An MIM code is assigned for around 55% of variants in each dataset.  
(2) By associating it with a gene. All variants are associated with genes, but variants may be designated as pathogenic for non-ACMG conditions.  
<br /> <br />
...
getAlleleFreq <- function(input, ind, dataset, method) {
  ...
  if (toupper(method) == "MEDGEN") {
    ...
    report.use = report.mg; inherit.use = inheritance.mg
  }
  if (expand)
...
freq_1000g.count.mg <- getAlleleFreq(input = merged_1000g, ind = F, dataset = "1000G", method = "MedGen")
freq_1000g.calc.mg <- getAlleleFreq(input = merged_1000g, ind = T, dataset = "1000G", method = "MedGen")
freq_gnomad.calc.mg <- getAlleleFreq(input = merged_gnomad, ind = T, dataset = "GNOMAD", method = "MedGen")
freq_exac.calc.mg <- getAlleleFreq(input = merged_exac, ind = T, dataset = "EXAC", method = "MedGen")
...
save(freq_1000g.calc.gene, freq_1000g.count.gene, freq_exac.calc.gene, freq_gnomad.calc.gene, 
     ...
     freq_1000g.calc.mg, freq_1000g.count.mg, freq_exac.calc.mg, freq_gnomad.calc.mg, 
...
plot_af_relation <- function() {
  ...
  ggplot(aes(x = GENE, y = MIM, label = LABEL)) + 
    ...
    geom_abline(slope = 1, intercept = 0) + ggtitle("MIM vs. GENE") + 
    xlab(sprintf("AF using %s"),input1) + ylab(sprintf("AF using %s", input2))
}
plot_af_relation(freq_gnomad.calc.gene$AF_GNOMAD, freq_gnomad.calc.mim$AF_GNOMAD)
plot_af_relation(freq_gnomad.calc.gene$AF_GNOMAD, freq_gnomad.calc.mg$AF_GNOMAD)
...
plot_ind_test <- function() {
  ...
  cat("\nThis plot has been downsampled 10x and contains 3,000 points.")
