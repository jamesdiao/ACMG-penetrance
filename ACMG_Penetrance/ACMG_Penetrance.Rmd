...
# save.image("/Users/jamesdiao/Documents/Kohane_Lab/2016-paper-ACMG-penetrance/Environ_2016-11-23.RData")
# load("/Users/jamesdiao/Documents/Kohane_Lab/2016-paper-ACMG-penetrance/Environ_2016-11-23.RData")
...
ACMG.table[1:5,] %>% select(Phenotype, Typical_age_of_onset, Gene, Inheritance, Variants_to_report) %>% pander
cat("ACMG-56 Genes:")
...
```{r get_lit}
ACMG_Lit <- read.csv(file = "Supplementary_Files/Literature_Prevalence_Estimates.csv", 
              header = TRUE, stringsAsFactors = F, na.strings = "\\N")
abbrev <- ACMG_Lit$Short_Name
prevalence_1 <- sapply(ACMG_Lit$Inverse_Prevalence %>% strsplit("-"), "[", 1) %>% as.numeric
prevalence_2 <- sapply(ACMG_Lit$Inverse_Prevalence %>% strsplit("-"), "[", 2) %>% as.numeric
expand <- function(item) { strsplit(item, "|", fixed = T) %>% unlist }
gene.list <- expand(ACMG_Lit$Gene)
report <- setNames(ACMG_Lit$Variants_to_report %>% expand, gene.list)[!duplicated(gene.list)]
inheritance <- setNames(ACMG_Lit$Inheritance %>% expand, gene.list)[!duplicated(gene.list)]
combined <- (prevalence_1+prevalence_2)/2
mean.prev <- prevalence_1 %>% setNames(abbrev)
...
ACMG_Lit[c(4,5,8,14),] %>% remove_rownames %>% 
  select(Gene, Phenotype, Abbreviation, Inverse_Prevalence) %>% pander
acmg_ah <- ACMG_Lit$Allelic_Heterogeneity %>% strsplit("-") %>% 
  sapply(as.numeric) %>% sapply(mean)
```
<br />
...
getAlleleFreq <- function(input, ind, method, dataset) {
  ...
  output <- sapply(search_list, function(item.vec) {
    item.vec.split <- expand(item.vec)
    loc <- rep(FALSE, nrow(input)) #loc = locations of all the "hits"
    for(item in item.vec.split) {
      keep <- rep(T, nrow(input)) #Normally, take EP and KP
      if (!grepl("EP",report[[item]])) #If we're only taking KP
        keep <- KP_only
      loc <- loc | (grepl(item,input[,search_in], ignore.case = T) & keep) #Take all relevant genes
    }
    hits <- sum(loc)
    sapply(c(dataset,super.levels), function(superpop) {
      if (ind) {
        ...
        if (inheritance[item] == "AR")
          freq <- freq^2 #AR: prob of having non-references sites at BOTH alleles
        if (inheritance[item] %in% c("AD","SD","XL"))
          freq <- 1-(1-freq)^2 #AD/SD/SL: prob of having a non-reference site at EITHER allele
        ...
        if (inheritance[item] == "AR")
          final <- mean( apply(input[loc, catch], 2, function(col) any(col==2)) )
        if (inheritance[item] %in% c("AD","SD","XL"))
...
ggplot(aes(x = CALC_1000G, y = CALC_EXAC), data = allele.freq) + 
  ...
  scale_x_continuous(limits = c(10^-6, max(allele.freq[,"CALC_EXAC"]))*5, 
                     trans='log10', breaks = 10^-(0:6)) + 
  scale_y_continuous(limits = c(10^-6, max(allele.freq[,"CALC_EXAC"]))*5, 
                     trans='log10', breaks = 10^-(0:6)) + 
  xlab("Allele Frequency (1000 Genomes)") + ylab("Allele Frequency (ExAC)") +
  geom_abline(slope = 1, intercept = 0) + ggtitle("Scatterplot: ExAC v. 1000 Genomes")
```

\newpage
Ratio_1000G (red, top) computes AF(calculation in 1000 Genomes) / AF(counting in 1000 Genomes).  
Ratio_ExAC (blue, bottom) computes AF(calculation in ExAC) / AF(calculation in 1000 Genomes).  
...
ratio_diff <- function() {
  ratio <- data.frame(Means = allele.freq$COUNT_1000G, 
    ...
    mutate(Disease = factor(abbrev, 
           levels = abbrev[order(pmax(Ratio_ExAC,Ratio_1000G))])) %>% 
...
plot_ind_test <- function() {
  ...
  #  data.frame(DISEASE = ACMG_Lit$Abbreviation,
...
Mean ratio (Calculation/Counting): `r signif(mean(plot_ind_out$CALC/plot_ind_out$COUNT, na.rm = T),3)`
...
get_penetrance <- function(ah_low, ah_high, dataset) {
  ...
  named.prev <- 1/mean.prev %>% setNames(abbrev)
  ...
  order(penetrance %>% colSums, decreasing = T) -> ord
...
pen_1000g <- get_penetrance(ah_low = 0.001, ah_high = 1, dataset = "1000 Genomes")
pen_exac <- get_penetrance(ah_low = 0.001, ah_high = 1, dataset = "ExAC")
```
<br />
...
#doubles <- ACMG_Lit[!is.na(combined),c("Short_Name","Inverse_Prevalence_1","Inverse_Prevalence_2")]
doubles <- data.frame("Disease" = ACMG_Lit$Short_Name, "Inverse_Prevalence_1" = prevalence_1, "Inverse_Prevalence_2" = prevalence_2) %>% filter(!is.na(Inverse_Prevalence_2))
doubles$Mean_Inverse_Prevalence <- apply(doubles[,-1], 1, function(row) 1/mean(1/row))
data.frame(Disease = doubles$Disease, Prevalence_Ratio = round(doubles$Inverse_Prevalence_2/doubles$Inverse_Prevalence_1,1)) %>% arrange(Prevalence_Ratio) %>% mutate(Prevalence_Ratio = sprintf("%.1f",Prevalence_Ratio)) %>% pander
```
...
make_prev_ranges <- function(range) {
    sapply(ACMG_Lit$Inverse_Prevalence %>% strsplit("-"), function(prev){
    if (length(prev)>1) {
...
get_penetrance <- function(ah_low, ah_high, dataset, range) {
  ...
  set_to_na <- function(row) { replace(row, is.infinite(row),NA) %>% pmin(1)}
  penetrance <- apply(prev_final*allelic.het / named.freqs, 1, set_to_na) %>% as.data.frame
  colnames(penetrance) <- abbrev
  # Matrix of penetrance values for allelic het range, capped at 1
  order(colSums(penetrance), decreasing = T) -> ord
  # replicate each element n times to create labels
  penetrance_data <- data.frame("Penetrance" = unlist(penetrance), "Disease" =
    factor(sapply(abbrev, function(x) rep(x,5)) %>% as.vector,
    levels = abbrev[ord]) )
  colormap <- rep('black', length(abbrev))
  colormap[1:(mean(is.na(penetrance_data$Penetrance))*length(colormap))] <- 'gray60'
...
pen_1000g_max <- get_penetrance(ah_low = 0.01, ah_high = 1, dataset = "1000 Genomes", range = 5)
pen_exac_max <- get_penetrance(ah_low = 0.01, ah_high = 1, dataset = "ExAC", range = 5)
#ah_low = 0.01; ah_high = 1; dataset = "ExAC"; range = 5
```
...
ancestry_penetrance(ah_low = 0.01, ah_high = 1, dataset = "1000 Genomes", range = 5, position = "Max")
ancestry_penetrance(ah_low = 0.01, ah_high = 1, dataset = "ExAC", range = 5, position = "Max")
#ah_low = 0.01; ah_high = 1; dataset = "ExAC"; range = 5; position = "Max"
```
...
Max penetrance values computed using 1000 Genomes are `r signif(lm_compare$coefficients[2],3)`-fold larger than those computed using ExAC.   
