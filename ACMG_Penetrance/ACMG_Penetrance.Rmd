...
```{r distr_prev, fig.width = 6, fig.height = 3.5}
...
ggplot(aes(x = factor(Disease, levels = Disease[order(Inverse_Prevalence)]), y = Inverse_Prevalence)) + 
  ...
  ggtitle("Distribution of Inverse Prevalences (log-log scale)") + ylab("Inverse Prevalence")
```
...
```{r af_scatter, fig.width = 4.5, fig.height = 3.5}
...
ggplot(aes(x = CALC_1000G, y = CALC_EXAC), data = allele.freq) + 
  ...
  geom_text_repel(aes(label = disease_abbrev_longer), size = 2.5) + 
...
Repeat 10 times and plot the disease_allele frequencies (10 of each).   
...
plot_ind_test <- function() {
  ...
  plot(ggplot(aes(x = COUNT, y = CALC), data = ind_test_data) + geom_point() + 
         ...
         geom_text(aes(label = replace(DISEASE,which(abs(COUNT - CALC)<0.1), NA)), 
                   check_overlap = T, hjust = 1, na.rm = T, size = 2.5)
       )
  ind_test_data
}
...
get_penetrance <- function(ah_low, ah_high, dataset) {
  ...
  plot(ggplot(aes(x=Disease, y=Penetrance), data = penetrance_data) + 
    geom_boxplot(position = 'identity', coef = 0, na.rm = T) + coord_flip() + ggtitle(dataset)
  )
  penetrance_data
}
...
Note 1: the empty lines at the top all indicate no allele frequency (disease_AF) data.  
...
ancestry_penetrance <- function(ah_low, ah_high, dataset, range) {
  ...
  plot(ggplot(aes(x=Disease, y = Subset), data = penetrance_data) + coord_flip() + 
    geom_tile(aes(fill = Penetrance), color = 'white') + xlab("Ancestry") + 
    scale_fill_gradient(low='white',high = 'steelblue') + 
    ggtitle(sprintf("Heatmap: Penetrance by Ancestry (%s)", dataset))
  )
  plot(penetrance_data[c(F,F,T,F,F),] %>% 
    ggplot(aes(x = Subset, y = Penetrance, group = Disease, color = Disease)) + 
    theme(legend.position = 'bottom', 
          legend.text = element_text(size = 7),
          legend.title = element_blank()) + 
    geom_line(na.rm = T) + xlab("Ancestry") + 
    ggtitle(sprintf("Parallel Coordinates: Penetrance by Ancestry (%s)", dataset))
  )
}
