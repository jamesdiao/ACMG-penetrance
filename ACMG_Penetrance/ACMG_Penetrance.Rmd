...
#save.image("/Users/jamesdiao/Documents/Kohane_Lab/2017-ACMG-penetrance/MA_Environ_2017-05-23.RData")
#load("/Users/jamesdiao/Documents/Kohane_Lab/2017-ACMG-penetrance/MA_Environ_2017-05-23.RData")
```

```{r packages, include = F, cache = F}
# Install and load all required packages
pkg_list <- c("scrapeR","RMySQL", "knitr","pander","ggplot2",
              "ggrepel", "tibble","curl","tidyr","dplyr")
...
get_mc_clinvar <- function(clinvar_file) {
  ...
  input <- input[nchar(input$REF)==1,] #deletions
  split_all <- strsplit(input$INFO,";")
  input$pathogenic <- grepl(1,sapply(split_all, function(x) x[grep('PATHOGENIC=',x)]))
  input$benign <- grepl(1,sapply(split_all, function(x) x[grep('BENIGN=',x)]))
  input$GOLD_STARS <- input$INFO %>% strsplit("GOLD_STARS=") %>% 
    sapply('[[',2) %>% unlist %>% strsplit(';') %>% 
    sapply('[[',1) %>% unlist %>% as.integer
  input$CLNSIG <- input$INFO %>% strsplit("CLINICAL_SIGNIFICANCE=") %>% 
    sapply('[[',2) %>% unlist %>% strsplit(';') %>% 
    sapply('[[',1) %>% unlist %>% strsplit('|', fixed = T)
  input$INTERP <- input$pathogenic
  input$CLNALLE <- 1
  input$CLNREVSTAT <- 1
  input$CLNDSDBID <- 1
  input$CLNDSDB <- 1
...
collect <- clinvar$INTERP #& clinvar$LMM #(clinvar$GOLD_STARS >= 2)
...
```{r pathogenic_var_plot, fig.height = 4, include = F}
