...
pars_from_file <- function(path) {
  byline <- readLines(path)
  byline <- byline[!grepl("###",byline)]
  lapply(byline, function(x) {
    eval(parse(text=x))
  })
}
path = "/Users/jamesdiao/Documents/Kohane_Lab/2016-paper-ACMG-penetrance/ClinVar_Reports/ClinVar_Parameters.txt"
if ("ClinVar_Parameters.txt" %in% system("ls", intern = T)) {
  pars_from_file(path)
} else {
  #Input
