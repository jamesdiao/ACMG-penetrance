...
#save.image("/Users/jamesdiao/Documents/Kohane_Lab/2016-paper-ACMG-penetrance/Environ_2016-10-25")
#load("/Users/jamesdiao/Documents/Kohane_Lab/2016-paper-ACMG-penetrance/Environ_2016-10-25")
...
pkg_list <- c("tidyverse","scrapeR","curl","RMySQL", "knitr","pander")
...
ACMG.table[c(1,10,12,14,18,20,60),] %>% remove_rownames %>% pander
cat("ACMG-56 Genes:")
...
cat(sprintf("Processed ClinVar data frame %s x %s (selected rows and columns):", nrow(clinvar), ncol(clinvar)))
...
cat("File saved as download_output.txt in Temp_Files")
```
<br />
...
cat(sprintf("Processed 1000 Genomes VCFs: %s x %s (selected rows and columns):", nrow(ACMG.1000g), ncol(ACMG.1000g)))
...
cat(sprintf("Processed ExAC VCFs: %s x %s (selected rows and columns):", nrow(ACMG.exac), ncol(ACMG.exac)))
...
`clinvar_query.txt` contains all results matched by the search query: "(APC[GENE] OR MYH11[GENE]... OR WT1[GENE]) AND (clinsig_pathogenic[prop] OR clinsig_likely_pathogenic[prop])" from the ClinVar website. The exact query is saved in /Temp_Files/query_input.txt  
<br /> <br />
...
```{r clinvar_query}
clinvar_query <- read.table(file = "Temp_Files/clinvar_query_2016-10-25.txt", sep = "\t", header = F, skip = 1, stringsAsFactors = F)
...
data.frame(Subset = c("Initial Count","Filter Substitutions (N>N')","Filter Coupling/Bad-Locations","In ClinVar VCF","In LP/P-ClinVar VCF","In LP/P-ClinVar VCF & ACMG & ExAC","In LP/P-ClinVar VCF & ACMG & 1000 Genomes", "In LP/P-ClinVar VCF & ACMG & ExAC & 1000 Genomes"), Number_of_Variants = clinvar_count) %>% pander
```
<br /> <br /> 
...
The median AF(disease) ratio between counting and calculation is: `r signif(count_calc_diff,4)`.  
The median AF(disease) ratio between ExAC and 1000 Genomes is: `r signif(exac_1000g_diff,4)`.  
<br />
