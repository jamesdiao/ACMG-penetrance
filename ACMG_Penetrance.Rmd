...
####Extract relevant entries from INFO:
...
(b) CLNSIG = "Variant Clinical Significance, 0 - Uncertain significance, 1 - not provided, 2 - Benign, 3 - Likely benign, 4 - Likely pathogenic, 5 - Pathogenic, 6 - drug response, 7 - histocompatibility, 255 - other"
...
(c) For ExAC: African = AFR, Latino = AMR, European (Finnish + Non-Finnish) = EUR, East.Asian = EAS, South.Asian = SAS. 
(d) For 1000 Genomes: convert genomes to allele counts (0|1 becomes 1, 1|1 becomes 2. 0|2 becomes 2 counts: 0 and 1). 
(e) Create other useful columns for analysis.
...
freq_1000g.dep <- getAlleleFreq(merged_1000g, tags, ind = F, dataset = "1000G")
freq_1000g.indep <- getAlleleFreq(merged_1000g, tags, ind = T, dataset = "1000G")
freq_exac.indep <- getAlleleFreq(merged_exac, tags, ind = T, dataset = "EXAC")
...
allele.freq <- data.frame("Pattern" = tags %>% unique,
                   COUNT_1000G = freq_1000g.dep$AF_1000G, INDEP_1000G = freq_1000g.indep$AF_1000G, INDEP_EXAC = freq_exac.indep$AF_EXAC)
...
ACMG_Lit <- read.csv(file = "ACMG_Lit_Small.csv", header = TRUE, stringsAsFactors = F, na.strings = "\\N",
                     colClasses = colclass)
rm(colclass)
get_penetrance <- function(ah_low, ah_high){
  # Map of disease name to disease tags
  disease <- ACMG_Lit$Disease
  named.freqs <- disease_af %>% setNames(disease)
  ...
  data.frame("penetrance" = penetrance, "disease" =
    factor(sapply(disease, function(x) rep(x,length(allelic.het))) %>% as.vector,
    levels = disease[ord]) )
}
penetrance_data <- get_penetrance(ah_low = 0.001, ah_high = 0.5)
par(mar=c(5, 20, 5, 2))
boxplot(penetrance ~ disease, penetrance_data, horizontal = TRUE, las = 1, xlab = "Penetrance", main = "Penetrance Range Estimates for P(V|D)", cex.axis = 0.7)
abline(v=0.5, lty=2)
```

```{r pen_cdf} 
par(mar=c(5, 12, 5, 2))
penetrance_data$penetrance[seq(5,150,5)] %>% ecdf %>% 
...
barplot(values, horiz = T, las = 1, xlim = c(0,ceiling(max(values)/5)*5), xlab = "Ratio of prevalence high/low", main = "Range of prevalence estimates")
par(mar=c(5, 4, 4, 2)+0.1)
```
