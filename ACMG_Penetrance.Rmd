...
plot_af_distrib <- function() {
  af_distrib <- data.frame(Index = 2:max(nrow(merged_1000g),nrow(merged_exac))-1,
    ...
    AF_EXAC = sort(merged_exac$AF_EXAC[merged_exac$AF_EXAC != max(merged_exac$AF_EXAC)], 
                   decreasing = T)) 
  #if (plot.log)
  #  af_distrib <- af_distrib %>% mutate(AF_1000G = log10(AF_1000G), AF_EXAC = log10(AF_EXAC))
  af_distrib_full <- gather(af_distrib, Dataset, Allele_Frequency, AF_1000G, AF_EXAC)
  af_distrib_short <- gather(af_distrib[1:50,], Dataset, Allele_Frequency, AF_1000G, AF_EXAC)
  af_distrib <- data.frame(rbind(af_distrib_full, af_distrib_short), 
                           Zoom = rep("All_Data",nrow(af_distrib_full)) %>%
                              c(rep("Subset (Top 50)",nrow(af_distrib_short)))) %>% 
                filter(!is.na(Allele_Frequency))
  ggplot(aes(x = Index, y = Allele_Frequency, color = Dataset), data = af_distrib) + 
    geom_point() + facet_grid(Dataset ~ Zoom, scales="free_x") + xlab("Size Rank") + 
...
```{r ind_test, fig.height = 2, fig.width = 3}
plot_ind_test <- function() {
  set.seed(1234)
  select_rows <- sample(nrow(ACMG.1000g),10000)
  ind_test_data <- data.frame(COUNT = getAlleleFreq(ACMG.1000g[select_rows,], 
...
penetrance_out <- function(allelic.het) {
  ...
  plot(ggplot(aes(x=Disease, y=Penetrance), data = penetrance_data) + 
    ...
    ggtitle(sprintf("P(V|D) = %s", allelic.het))
  )
  penetrance_data
}
