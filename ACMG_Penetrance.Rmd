...
#save.image("/Users/jamesdiao/Documents/Kohane_Lab/2016-paper-ACMG-penetrance/Environ_2016-10-21")
#load("/Users/jamesdiao/Documents/Kohane_Lab/2016-paper-ACMG-penetrance/Environ_2016-10-21")
...
pkg_list <- c("dplyr","tidyr","ggplot2","scrapeR","curl","RMySQL")
...
cat("Processed Table from Website:")
print(ACMG.table[10:12,], quote = F)
cat("ACMG-56 Genes:")
...
(a) CLNALLE = "Variant alleles from REF or ALT columns.  0 is REF, 1 is the first ALT allele, etc.  This is used to match alleles with other corresponding clinical (CLN) INFO tags.  A value of -1 indicates that no allele was found to match a corresponding HGVS allele name."
(b) CLNSIG = "Variant Clinical Significance, 0 - Uncertain significance, 1 - not provided, 2 - Benign, 3 - Likely benign, <br />
4 - Likely pathogenic, 5 - Pathogenic, 6 - drug response, 7 - histocompatibility, 255 - other"
(c) CLNDBN = "Variant disease name"
(d) CLNDSDBID = "Variant disease database ID" 
...
clinvar[5,] %>% select(-CLNALLE, -INFO, -QUAL, -FILTER)
```
...
if (!skip_download & !skip_processing) {
  ...
  if (skip_download)
    ...
    download_output <- read.table("Temp_Files/download_output.txt", header = T, stringsAsFactors = F)
}
download_output[1:5,]
```

> 5. Import and Process ExAC VCFs
...
> 6. Import and Process 1000 Genomes VCFs
...
ACMG.1000g[1:5,1:15] %>% select(-INFO, -QUAL, -FILTER, -FORMAT)
```
...
data.frame(Number.of.Variants = 
  c(nrow(clinvar), sum(clinvar$INTERP), sum(apply(clinvar[clinvar$INTERP,], 1, is.acmg)), 
...
> 8. Compare with ClinVar website: merge with `clinvar_query.txt`, which all results matched by the search query: "(APC[GENE] OR MYH11[GENE]... OR WT1[GENE]) AND (clinsig_pathogenic[prop] OR clinsig_likely_pathogenic[prop])' from the ClinVar website. 
...
```{r clinvar_query}
clinvar_query <- read.table(file = "Temp_Files/clinvar_query.txt", sep = "\t", header = F, skip = 1, stringsAsFactors = F)
...
#  write.table(file = "Temp_Files/cvquery_location.bed", quote = F, row.names = F, col.names = F)
#system("chmod +x ../Tools/liftOver")
#system("../Tools/liftOver Temp_Files/cvquery_hg38.bed ../Tools/hg38ToHg19.over.chain.gz Temp_Files/cvquery_hg19.bed err.log")
cvquery_hg19 <- read.table(file = "Temp_Files/cvquery_hg19.bed", sep = "\t", header = F, stringsAsFactors = F)
clinvar_query <- clinvar_query %>%
...
merged_1000g[merged_1000g$VAR_ID %in% clinvar_query$VAR_ID, c(3,1:2,6,10)]
cat(sprintf("Overlap with LP/P ClinVar VCF AND ExAC: %s variants", sum(clinvar_query$VAR_ID %in% merged_exac$VAR_ID)))
merged_exac[merged_exac$VAR_ID %in% clinvar_query$VAR_ID, c(1:3,11,15)][1:10,]
```
...
```{r gene_distribution, fig.width = 10}
...
The mean number of non-reference sites is $E(V)$, where $V = \sum_{i=1}^n v_i$ is the number of non-reference sites at any variant position $v_1$ through $v_n$. <br />

At each variant site, the probability of having at least 1 non-reference allele is $P(v_i) = P(v_{i,a} \cup v_{i,b})$, where $a$ and $b$ indicate the 1st and 2nd allele at each site. <br />

If the two alleles are independent, $P(v_{i,a} \cup v_{i,b})$ = $1-(1-P(v_{i,a}))(1-P(v_{i,b})) = 1-(1-AF(v_i))^2$ <br />

If all variants are independent, $E(V) = \sum_{i=1}^n 1-(1-AF(v_i))^2$ for any set of allele frequencies. Note: this is not true in all cases, e.g., when multiple variants share the same position. However, exceptions such as these are rare and can be safely ignored. <br /> 

Since we have the allele frequencies for each superpopulation, we can estimate $E(V)$ for each superpopulation. <br /> <br /> 

Ex: the allele frequencies of 3 variants across the 5 superpopulations looks like this: 
```{r calc_example}
example <- rbind(c(0.1,0.2,0,0,0.3),c(0.2,0,0.3,0,0.1),c(0,0,0.1,0.1,0.2))
rownames(example) <- c("Variant 1", "Variant 2","Variant 3")
colnames(example) <- sprintf("AF_EXAC_%s",super.levels)
example
```
...
The expected (mean) number of non-reference sites is given by $\sum (columns)$. 
```{r} 
colSums(1-(1-example)^2)
```
...
> 4. Fraction of 1000 Genomes Individuals with Pathogenic Sites
...
```{r} 
apply(example, 2, function(x) 1-prod((1-x)^2))
```
...
> 7. Ethnic Breakdown of 1000 Genomes Pathogenic Variants
...
ACMG_Lit <- read.csv(file = "ACMG_Lit_Small.csv", header = TRUE, stringsAsFactors = F, na.strings = "\\N", colClasses = colclass)
ACMG_Lit[5,1:15]
```
...
This can be computed in two ways: <br />
(a) By direct counting, from genotype data in 1000 Genomes. <br />
(b) AF(disease) = $1-\prod_{variant}(1-AF_{variant})$, from population data in ExAC. This assumes independence between variants. <br />

```{r lit_af}
getAlleleFreq <- function(input, tags, ind, dataset) {
  data <- toupper(dataset)
  temp <- sapply(unique(tags), function(tag) {
    ...
    hits <- sum(loc)
    if (ind) {
      freq <- input[loc,] %>% select(contains(data), contains("AF")) %>% unlist %>% as.numeric
      final <- 1-prod(1-freq[!is.na(freq)])
    } else {
      final <- mean(colSums( input[loc,(1+length(front_cols)):ncol(input)] ))/2
    }
    c(final, hits) %>% setNames(c(paste("AF",data,sep = "_"),"Hits"))
  }) %>% t %>% tbl_df
  data.frame("Tags" = unique(tags), temp)
}
freq_1000g.count <- getAlleleFreq(merged_1000g, tags, ind = F, dataset = "1000G")
freq_1000g.calc <- getAlleleFreq(merged_1000g, tags, ind = T, dataset = "1000G")
freq_exac.calc <- getAlleleFreq(merged_exac, tags, ind = T, dataset = "EXAC")
allele.freq <- data.frame("Pattern" = tags %>% unique,
                   COUNT_1000G = freq_1000g.count$AF_1000G, 
                   CALC_1000G = freq_1000g.calc$AF_1000G, 
                   CALC_EXAC = freq_exac.calc$AF_EXAC)
```

> 4. Comparisons of AF(disease) by dataset or method

Correlation Table: 
``` {r af_comparison, fig.width = 10}
cor(allele.freq[,2:ncol(allele.freq)])
par(mfrow=c(1,2))
#plot(allele.freq[,3:ncol(allele.freq)], main = "ExAC v. 1000 Genomes", xlim = c(0.001, max(allele.freq[,4])), las = 1); abline(a = 0, b = 1); frame()
pattern_short <- strsplit(allele.freq$Pattern %>% as.character, ";") %>% sapply("[[",1)
ggplot(aes(x = CALC_1000G, y = CALC_EXAC), data = allele.freq) + 
  geom_point(stat = "identity") + ggtitle("ExAC v. 1000 Genomes") + 
  geom_text(aes(label = pattern_short), check_overlap = T, hjust = 0, nudge_x = 0.005) + 
  scale_x_continuous(limits = c(0, max(allele.freq[,4])))
ratio_diff <- function(af_1, af_2, title) {
  (pmax(af_1, af_2)/pmin(af_1, af_2)) %>% setNames(pattern_short) %>% sort -> ratio
  plot(ratio, main = title, ylab = "Ratio (larger/smaller)")
  median(ratio[is.finite(ratio)])
}
count_calc_diff <- ratio_diff(allele.freq$CALC_1000G, allele.freq$COUNT_1000G, title = "Counting v. Calculation (1000 Genomes)")
exac_1000g_diff <- ratio_diff(allele.freq$CALC_EXAC, allele.freq$CALC_1000G, title = "1000 Genomes v. ExAC (Calculation)") %>% invisible
par(mfrow=c(1,1), mar=c(5, 4, 4, 2)+0.1)
```

The median AF(disease) ratio between counting and calculation is: `r signif(count_calc_diff,4)`. <br />
The median AF(disease) ratio between ExAC and 1000 Genomes is: `r signif(exac_1000g_diff,4)` <br />
...
get_penetrance <- function(ah_low, ah_high, dataset) {
  ...
  if (dataset == "1000 Genomes")
    named.freqs <- allele.freq$COUNT_1000G %>% setNames(disease)
  if (dataset == "ExAC")
    named.freqs <- allele.freq$CALC_EXAC %>% setNames(disease)
  ...
  penetrance_data <- data.frame("penetrance" = penetrance, "disease" =
    ...
    levels = disease[ord]) )
  par(mar=c(5, 20, 5, 2))
  boxplot(penetrance ~ disease, penetrance_data, horizontal = TRUE, las = 1, xlab = "Penetrance",
          main = paste(dataset,"Penetrance Estimates as a function of P(V|D)"), cex.axis = 0.7)
  abline(v=0.5, lty=2)
  par(mfrow=c(1,1), mar=c(5, 4, 4, 2)+0.1)
  penetrance_data
}
...
doubles <- ACMG_Lit[ranged <- !is.na(ACMG_Lit$Inverse.P2),1:5]
par(mar=c(5, 15, 5, 2))
values <- doubles$Inverse.P1/doubles$Inverse.P2 %>% setNames(doubles$Disease)
barplot(values %>% sort, horiz = T, las = 1, xlim = c(0,ceiling(max(values)/5)*5), 
        xlab = "Ratio of prevalence (larger/smaller)", main = "Range of prevalence estimates")
par(mfrow=c(1,1), mar=c(5, 4, 4, 2)+0.1)
```

``` {r disp_prev_plots, fig.width = 10}
...
penetrance_out <- function(allelic.het) {
  ...
  # replicate each element n times to create labels
  penetrance_data <- data.frame("penetrance" = penetrance,
    "disease" = factor(sapply(doubles$Disease, function(x) rep(x,5)) %>% as.vector,
    levels = doubles$Disease[ord])
  )
  par(mar=c(5, 15, 5, 2))
  boxplot(penetrance ~ disease, penetrance_data, horizontal = TRUE, las = 1, range = 0, 
          xlab = "Penetrance", ylim = c(0,1),
          main = paste("Penetrance Range Estimates for Prevalence Ranges, P(V|D) =", allelic.het))
  abline(v=0.5, lty=2)
  par(mfrow=c(1,1), mar=c(5, 4, 4, 2)+0.1)
  penetrance_data
}
...
```{r}
#download.file(url = "http://www.omim.org/static/omim/data/mim2gene.txt", destfile = "Temp_Files/mim2gene.txt", method = "internal")
#mim2gene <- read.table(file = "Temp_Files/mim2gene.txt", header = FALSE, sep = "\t", stringsAsFactors = F)
