...
#http://gnomad.broadinstitute.org/gene/ENSG00000134571
```

```{r packages, include = F, cache = F}
# Install and load all required packages
pkg_list <- c("scrapeR","RMySQL", "knitr","pander","ggplot2","tibble","curl","tidyr","dplyr")
...
#use_clinvar <- "Supplementary_Files/clinvar_2016-10-04.vcf"
...
if(has_internet()) {
  ...
  if (is.null(use_clinvar)) {
    clinvar_file <- sprintf("Supplementary_Files/clinvar_%s.vcf.gz", Sys.Date())
    ...
    clinvar_file <- sprintf("Supplementary_Files/clinvar_%s.vcf", Sys.Date())
  } else {
...
get_clinvar <- function(clinvar_file) {
  ...
  system(sprintf("mv %s Supplementary_Files/clinvar_%s.vcf", clinvar_file, file_date))
...
There are three ways of identifying which disease a variant belongs to:  
(1) By gene(s)  
(2) By disease MIM(s)  
(3) By keyword(s)  
...
getAlleleFreq <- function(input, ind, method, dataset) {
  ...
  output <- sapply(search_list, function(item.vec) {
    ...
    hits <- sum(loc)
    if (ind) {
      freq <- input[loc,] %>% select(contains(toupper(dataset)), contains("AF")) %>% unlist %>% as.numeric
      final <- 1-prod(1-freq[!is.na(freq)])
    } else {
      final <- mean(colSums( input[loc,(1+length(front_cols)):ncol(input)] ))/2
    }
    c(final, hits) %>% setNames(c(paste("AF",toupper(dataset),sep = "_"),"Hits"))
  }) %>% t %>% tbl_df
  data.frame(search_list, output)
}
freq_1000g.count.mim <- getAlleleFreq(merged_1000g, ind = F, method = "MIM", dataset = "1000G")
freq_1000g.calc.mim <- getAlleleFreq(merged_1000g, ind = T, method = "MIM", dataset = "1000G")
freq_exac.calc.mim <- getAlleleFreq(merged_exac, ind = T, method = "MIM", dataset = "EXAC")
freq_1000g.count.tags <- getAlleleFreq(merged_1000g, ind = F, method = "Tags", dataset = "1000G")
freq_1000g.calc.tags <- getAlleleFreq(merged_1000g, ind = T, method = "Tags", dataset = "1000G")
freq_exac.calc.tags <- getAlleleFreq(merged_exac, ind = T, method = "Tags", dataset = "EXAC")
...
allele.freq <- data.frame(
                   #MIM_COUNT_1000G = freq_1000g.count.mim$AF_1000G, 
                   #MIM_CALC_1000G = freq_1000g.calc.mim$AF_1000G, 
                   #MIM_CALC_EXAC = freq_exac.calc.mim$AF_EXAC, 
                   TAGS_COUNT_1000G = freq_1000g.count.tags$AF_1000G, 
                   TAGS_CALC_1000G = freq_1000g.calc.tags$AF_1000G, 
                   TAGS_CALC_EXAC = freq_exac.calc.tags$AF_EXAC
                   #GENE_COUNT_1000G = freq_1000g.count.gene$AF_1000G, 
                   #GENE_CALC_1000G = freq_1000g.calc.gene$AF_1000G, 
                   #GENE_CALC_EXAC = freq_exac.calc.gene$AF_EXAC
                   )
...
<!--\newpage-->
## Comparisons of AF(disease) by Dataset and Collection Methods
...
ggplot(aes(x = TAGS_CALC_1000G, y = TAGS_CALC_EXAC), data = allele.freq) + 
  geom_point(stat = "identity") + ggtitle("ExAC v. 1000 Genomes") + 
  geom_text(aes(label = row.names(allele.freq)), check_overlap = T, hjust = 0, nudge_x = 0.005) + 
  scale_x_continuous(limits = c(0, max(allele.freq[,"TAGS_CALC_EXAC"])))
ratio_diff <- function(af_1, af_2, title) {
  (pmax(af_1, af_2)/pmin(af_1, af_2)) %>% setNames(ACMG_Lit$Disease) %>% sort -> ratio
  plot(ratio, main = title, ylab = "Ratio (larger/smaller)")
  median(ratio[is.finite(ratio)])
}
count_calc_diff <- ratio_diff(allele.freq$TAGS_CALC_1000G, allele.freq$TAGS_COUNT_1000G, title = "Counting v. Calculation (1000 Genomes)")
exac_1000g_diff <- ratio_diff(allele.freq$TAGS_CALC_EXAC, allele.freq$TAGS_CALC_1000G, title = "1000 Genomes v. ExAC (Calculation)") %>% invisible
par(mfrow=c(1,1), mar=c(5, 4, 4, 2)+0.1)
```
...
the bold line in the middle indicates P(V|D) = 0.02,   
the right end of the boxplot indicates P(V|D) = 0.5.  

```{r penetrance_f_AH, fig.width = 8}
get_penetrance <- function(ah_low, ah_high, dataset) {
  ...
  if (dataset == "1000 Genomes")
    named.freqs <- allele.freq$TAGS_COUNT_1000G %>% setNames(disease)
  if (dataset == "ExAC")
    named.freqs <- allele.freq$TAGS_CALC_EXAC %>% setNames(disease)
  named.prev <- 1/mean.prev %>% setNames(disease)
...
<!--\newpage-->
## Penetrance as a Function of Prevalence

Note that this is only possible in the dozen or so cases where a prevalence range was given, rather than a point estimate. 
``` {r prev_range, fig.wigth = 8, fig.height = 4}
doubles <- ACMG_Lit[!is.na(combined),c("Disease","Inverse.Prevalence.1","Inverse.Prevalence.2")]
doubles$Mean.Inverse.Prevalence <- (doubles[,2]+doubles[,3])/2

data.frame(Disease = doubles$Disease, Prevalence_Ratio = round(doubles$Inverse.Prevalence.1/doubles$Inverse.Prevalence.2,1)) %>% arrange(Prevalence_Ratio) %>% pander
#par(mar=c(5, 15, 5, 2))
#values <- doubles$Inverse.Prevalence.1/doubles$Inverse.Prevalence.2 %>% setNames(doubles$Disease)
...
``` {r disp_prev_plots, fig.width = 8, fig.height = 3.5}
prev.range <- 1/doubles[,c(2,2,4,3,3)] %>% as.matrix
#Set this to 1 rather than 0.02
penetrance_out <- function(allelic.het) {
  penetrance <- (allelic.het*prev.range/allele.freq$TAGS_COUNT_1000G[!is.na(combined)]) %>% 
    t %>% as.vector %>% pmin(1)
  ...
  boxplot(penetrance ~ disease, penetrance_data, horizontal = TRUE, las = 1, range = 0, 
          ...
          main = paste("Penetrance Estimates for Prevalence Ranges, P(V|D) =", allelic.het))
  abline(v=0.5, lty=2)
  par(mfrow=c(1,1), mar=c(5, 4, 4, 2)+0.1)
  penetrance_data
}
...
<!--
```{r, eval = F}
real_tags <- unique(tags)
sapply(1:length(real_tags), function(i) {
  sapply(strsplit(ACMG_Lit$Disease_MIM[i],";")[[1]], function(mim) {
    grepl(mim, clinvar$CLNDSDBID)
  }) %>% apply(1,any) -> loc_1
  sapply(strsplit(real_tags[i],";")[[1]], function(mim) {
    grepl(mim, clinvar$CLNDBN)
  }) %>% apply(1,any) -> loc_2
  c(sum(loc_1),sum(loc_2))
})
good_results <- temp
colnames(good_results) <- ACMG_Lit$Disease
row.names(good_results) <- c("MIM","Tag")
```
-->
<!--
```{r, eval = F}
...
sapply(1:length(real_tags), function(i) {
  ...
  clinvar$CLNDBN[loc_1][sample(sum(loc_1),20)]
  
  length(grep("aneurysm",clinvar$CLNDBN[loc_1]))/sum(loc_1)
  grepl()
  
  length(grep("C0027672",clinvar$CLNDSDBID[loc_1]))/sum(loc_1)
  grepl()
  length(grep("C0027672",clinvar$CLNDSDBID[loc_1]))/sum(loc_1)
