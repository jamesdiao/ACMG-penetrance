...
output:
  ...
  html_document:
    ...
    toc: yes
geometry: margin=1in
abstract: "Here, we examine ExAC and the 1000 Genomes Project to investigate the distribution of pathogenic variants across diverse populations. Pathogenic variants were found to be distributed unevenly across ancestral groups, with incidental findings inflated relative to empirical disease prevalences. Quantitative risk estimates were derived by modeling penetrance as a function of disease prevalence, allele frequency, and allelic heterogeneity. Plausible ranges for these parameters were estimated from ExAC, the 1000 Genomes cohort, and the medical literature. Under the most generous assumptions, penetrance estimates for the majority of diseases fall under 50%, with many under 5%. We propose the described model of penetrance as a quantitative framework for evaluating, comparing, and updating clinical interpretations of pathogenic variants."
---
...
# save.image("/Users/jamesdiao/Documents/Kohane_Lab/2016-paper-ACMG-penetrance/Environ_2016-10-29")
# load("/Users/jamesdiao/Documents/Kohane_Lab/2016-paper-ACMG-penetrance/Environ_2016-10-29")
...
## Bayes' Rule as a Model for Estimating Penetrance Using 
...
```{r distr_prev, fig.width = 10, fig.height = 5}
par(mfrow=c(1,2), mar=c(5, 5.5, 4, 2)+0.1)
plot((1/mean.prev) %>% sort, 1:nrow(ACMG_Lit), 
     xlab = sprintf("Allele Frequency: (r=%s)", round(cor((1/mean.prev) %>% sort, 1:nrow(ACMG_Lit)),2)), 
     yaxt = 'n', ylab = "")
axis(2,at=1:nrow(ACMG_Lit),labels=disease_abbrev[order(1/mean.prev)], las = 2, cex.axis = 0.8)
plot((1/mean.prev) %>% sort, 1:nrow(ACMG_Lit), 
     log = "x", yaxt = 'n', xaxt = 'n', ylab = "",
     xlab = sprintf("Allele Frequency (log-scale): r=%s", round(cor(log(1/mean.prev) %>% sort, 1:nrow(ACMG_Lit)),2)))
axis(2,at=1:nrow(ACMG_Lit),labels=disease_abbrev[order(1/mean.prev)], las = 2, cex.axis = 0.8)
...
ggplot(aes(x = CALC_1000G, y = CALC_EXAC), data = allele.freq) + 
  ...
  geom_text(aes(label = disease_abbrev), check_overlap = T, hjust = 0, nudge_x = 0.005) +
  scale_x_continuous(limits = c(10^-5, max(allele.freq[,"CALC_EXAC"]))*5, trans='log10', breaks = 10^-(0:3)) + 
  scale_y_continuous(limits = c(10^-5, max(allele.freq[,"CALC_EXAC"]))*5, trans='log10', breaks = 10^-(0:3)) 
ratio_diff <- function(af_1, af_2, title) {
  (pmax(af_1, af_2)/pmin(af_1, af_2)) %>% setNames(disease_abbrev) %>% sort -> ratio
  ratio <- ratio[is.finite(ratio)]
  plot(ratio, 1:length((ratio)), xlab = "Ratio (larger/smaller)", main = title, yaxt = 'n', ylab = "")
  axis(2,at=1:length(ratio),labels=names(ratio), las = 2, cex.axis = 0.8)
  median(ratio)
}
...
```{r ah_plots, fig.width = 8, fig.height = 4.7}
get_penetrance <- function(ah_low, ah_high, dataset) {
  ...
  boxplot(penetrance ~ disease, penetrance_data, horizontal = TRUE, las = 1, xlab = "Penetrance",
          main = dataset, cex.axis = 0.7, xaxt = 'n')
  axis(1, at = seq(0,1,0.1), breaks = seq(0,1,0.1))
  abline(v=0.5, lty=2)
  par(mfrow=c(1,1), mar=c(5, 4, 4, 2)+0.1)
  penetrance_data
}
...
Note: the bold black lines at 1.0 all indicate no allele frequency (disease_AF) data. 
...
the bold line in the middle indicates P(D) = mean(log(values)),   
...
``` {r prev_plots, fig.width = 8, fig.height = 3.4}
...
## Penetrance ranges given P(D) and P(V|D): 

The left end of the boxplot indicates P(D) AND P(V|D) = lower value,  
the bold line in the middle indicates P(D) AND P(V|D) = mean(log(values)),   
the right end of the boxplot indicates P(D) AND P(V|D) = upper value.  
```{r max_values, eval = F} 

get_penetrance <- function(dataset) {
  # Map of disease name to disease tags
  if (dataset == "1000 Genomes")
    named.freqs <- allele.freq$COUNT_1000G %>% setNames(disease_abbrev_longer)
  if (dataset == "ExAC")
    named.freqs <- allele.freq$CALC_EXAC %>% setNames(disease_abbrev_longer)
  prev.range <- 1/doubles[,c(2,2,4,3,3)] %>% as.matrix
  row.names(prev.range) <- disease_abbrev_longer
  # Repeats allow for correct quartile calculations
  #point estimate set to geometric mean
  allelic.het <- c(ah_low, ah_low, sqrt(ah_low*ah_high) %>% round(3), ah_high, ah_high)
  # Matrix of penetrance values for allelic het range, capped at 1
