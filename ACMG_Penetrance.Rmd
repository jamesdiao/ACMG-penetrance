...
> 2. Scrape ACMG gene panel from ClinVar at: 
...
if (is.null(use_clinvar)) {
  download.file(url = "ftp://ftp.ncbi.nlm.nih.gov/pub/clinvar/vcf_GRCh37/clinvar.vcf.gz", 
                destfile = sprintf("Temp_Files/clinvar_%s.vcf.gz", Sys.Date()), method = "internal")
...
> 4. Download 1000 Genomes VCFs (pulled from working directory)
...
> 5. Import ExAC (downloaded by hand) and 1000 Genomes 
...
if ("ExAC" %in% dir.files)
  system("rm -r ExAC")
if ("Temp_Files" %in% dir.files)
