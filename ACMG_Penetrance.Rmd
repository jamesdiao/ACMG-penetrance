...
```{r distr_prev, fig.width = 10, fig.height = 4}
par(mfrow=c(1,2), mar=c(5, 5.5, 1, 2)+0.1)
...
ratio_diff <- function() {
  ratio <- data.frame(Means = rowMeans(allele.freq[,-1]), 
                     ...
                     Label = disease_abbrev_longer) %>%
    gather(Dataset, Ratio, Ratio_1000G, Ratio_ExAC)
  ratio <- ratio[is.finite(ratio$Ratio),]
  ggplot(aes(x=Means, y=Ratio), data = ratio) + geom_point() + geom_text_repel(aes(label = Label)) + 
    facet_grid(. ~ Dataset)
  ggplot(aes(x=1:nrow(ratio), y=sort(Ratio)), data = ratio) + geom_point() + 
...
```{r ind_test, fig.height = 4, fig.width = 6}
plot_ind_test <- function() {
  ...
  plot(ggplot(aes(x = COUNT, y = CALC), data = ind_test_data) + geom_point() + 
         ...
         ggtitle("Counting v. Calculation w/ Independence From Random Sampling")
       )
  ind_test_data
}
