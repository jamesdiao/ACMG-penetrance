...
skip_processing <- "ACMG_1000G.RData" %in% system("ls",intern = T)
```
...
> 4. Download ClinVar Interpretations
...
if (!skip_download & !skip_processing) {
  ...
  download_output <- download_output %>%
    mutate(chrom = sapply(strsplit(download_output$chrom,"chr"), function(x) x[2]), 
           start = as.integer(start), end = as.integer(end), downloaded = as.logical(downloaded))
  system("rm *.genotypes.vcf.gz.tbi")
}
```

> 6. Import ExAC (downloaded by hand) and 1000 Genomes 
(supposedly downloaded just now)

```{r import_all}
import.file.exac <- function(gene) {
  name <- paste("ExAC",paste(gene,"exac.csv", sep = "_"), sep= "/")
...
merged <- cbind(ACMG_merged[,c("GENE","AF")], clinvar_merged,
                ACMG_merged[,grep("[0-9]",colnames(ACMG.1000g))])
#count up all pathogenic ClinVar in ACMG regions
if(skip_download) 
  download_output <- read.csv("download_output.csv", header = T, stringsAsFactors = F)
...
clinvar_bp <- c(nrow(clinvar), sum(clinvar$INTERP), 
                sum(apply(clinvar[clinvar$INTERP,], 1, is.acmg)), nrow(merged)) %>% 
  setNames(c("ClinVar","Patho","in ACMG","in 1000G"))
...
if ("1000G" %in% dir.files)
  system("rm -r 1000G")
if ("ACMG_1000G.RData" %in% dir.files)
  system("rm -r ACMG_1000G.RData")
```
