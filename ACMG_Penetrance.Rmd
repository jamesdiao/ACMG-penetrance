...
output:
  ...
  html_document:
    ...
    toc: yes
geometry: margin=1in
abstract: Here, we examine ExAC and the 1000 Genomes Project to investigate the distribution
  of pathogenic variants across diverse populations. Pathogenic variants were found
  to be distributed unevenly across ancestral groups, with incidental findings inflated
  relative to empirical disease prevalences. Quantitative risk estimates were derived
  by modeling penetrance as a function of disease prevalence, allele frequency, and
  allelic heterogeneity. Plausible ranges for these parameters were estimated from
  ExAC, the 1000 Genomes cohort, and the medical literature. Under the most generous
  assumptions, penetrance estimates for the majority of diseases fall under 50%, with
  many under 5%. We propose the described model of penetrance as a quantitative framework
...
# save.image("/Users/jamesdiao/Documents/Kohane_Lab/2016-paper-ACMG-penetrance/Environ_2016-11-01")
# load("/Users/jamesdiao/Documents/Kohane_Lab/2016-paper-ACMG-penetrance/Environ_2016-11-01")
...
getAlleleFreq <- function(input, ind, method, dataset) {
  ...
  output <- sapply(search_list, function(item.vec) {
    ...
    hits <- sum(loc)
    if (ind) {
      freq <- input[loc,] %>% select(contains(toupper(dataset)), contains("AF")) %>% unlist %>% as.numeric #vector of all allele frequencies
      ...
      final <- mean(colSums( input[loc,-front_cols] )>0)
    }
...
Correlation Table: 
``` {r af_comparison, fig.width = 10, dependson = "disease_afs"}
...
ggplot(aes(x = CALC_1000G, y = CALC_EXAC), data = allele.freq) + 
  geom_point(stat = "identity", col = 'red') + ggtitle("ExAC v. 1000 Genomes") + 
  geom_text_repel(aes(label = disease_abbrev_longer)) +
...
```{r indep, eval = F}
rowSums(ACMG.1000g[,-front_cols])==0 -> rm.rows
row.sub <- 1000
ACMG.1000g[1:row.sub, -front_cols] %>% filter(!rm.rows[1:row.sub]) %>% t %>% cor -> temp
plot(apply(temp, 1, function(x) max(x[x<1]) ))
```
